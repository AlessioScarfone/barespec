...
Run each instruction in order **after** the full workflow completes (including the final summary, which states the path of the generated `visual.html` so post-hooks can act on it). Announce each one:
