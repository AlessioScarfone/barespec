---
name: init-config
description: 'Scaffold or update the barespec configuration file (barespec.config.yml) that defines pre and post hooks for the context, spec, plan, implement, review, and visualize steps. Use when setting up hook customization, configuring hooks, adding new hooks, reviewing or editing hook settings, or when asked to "setup barespec", "configure hooks", "edit the config yml", or "add a pre-hook". Do not use for implementing features, writing specs, or creating project context.'
disable-model-invocation: true
---

Scaffold or update the barespec configuration file at `./barespec/barespec.config.yml`.

This file is read by `init-context`, `spec-create`, `spec-plan`, `spec-implement`, `spec-code-review`, and `spec-visualize` to inject custom pre/post instructions into their workflows.
...
1. Check whether `./barespec/barespec.config.yml` already exists.
   - **If it exists** → read it, show a summary of configured hooks:
     ...
     - visualize.pre:  N hook(s)
     - visualize.post: N hook(s)
     ```
     Then ask:
...
Use the script in [references/hook-interview.md](./references/hook-interview.md) to collect hook instructions. Ask all twelve questions in a single numbered message, pre-filling current values where a config already exists. Follow the answer-interpretation rules in the same file.
